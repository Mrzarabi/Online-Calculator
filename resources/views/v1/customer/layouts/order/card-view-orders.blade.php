        ...
        @foreach ($orders as $order)
            <div class="custom-background-card custom-card p-4 mb-2">
                <div class="card-body pb-0">
                    <h4 class="custom-font-size mb-1 color"> {{ $order->accept == false ? 'Pending' : 'Accepted' }}
                    </h4>
                    <h6 class="mr-2 color mb-4">Order Number: {{ $order->order_number }}</h6>
                    <div class="text-center w-100">
                        <div class="d-flex justify-content-center align-items-center">
                            <h6 class="mr-2 text-color">
                                {{ isset($order->calculator->name) ? $order->calculator->name : 'NO TEXT' }}</h6>
                            <h4 class="mr-2 text-color">{{ $order->input_number ? $order->input_number : 'NO TEXT' }}
                            </h4>
                            <h6 class="mr-2 text-color">
                        ...
                        <div class="d-flex justify-content-center align-items-center">
                            <h6 class="mr-2 text-color">
                                {{ isset($order->element->name) ? $order->element->name : 'NO TEXT' }}</h6>
                            <h4 class="mr-2 text-color">{{ $order->output_number ? $order->output_number : 'NO TEXT' }}
                            </h4>
                            <h6 class="mr-2 text-color">
                                {{ $order->output_currency_unit ? $order->output_currency_unit : 'NO TEXT' }}</h6>
                        </div>
                        <div class="d-flex justify-content-start">
                            <h6 class="mr-2 mb-2 custom-font-size text-color">
                                {{ Carbon::parse($order->created_at)->format('d/m/Y') }}</h6>
                        </div>
                    </div>
                </div>
                <div class="d-flex justify-content-center mb-2">
                    @if ($order->clearing)
                        <button type="button" class="btns btn-sm mr-1 text-color custom-mobile-font-size p-2"
                            data-toggle="modal" data-target="#mobile-clearing-{{ $order->clearing->id }}"
                            data-whatever="@mdo">Show Doc</button>
                    @endif
                    @if ($order->form)
                        <button type="button" class="btns btn-sm mr-1 text-color custom-mobile-font-size p-2"
                            data-toggle="modal" data-target="#mobile-form-{{ $order->form->id }}"
                            data-whatever="@mdo">Show Form</button>
                    @endif
                    @if ($order->accept == true)
                        <button type="button"
                            class="btns btn-sm text-color custom-mobile-font-size p-2">Accepted</button>

                        @if ($order->feedback)
                            <button type="button" class="btns btn-sm ml-1 text-color custom-mobile-font-size p-2"
                                data-toggle="modal" data-target="#mobile-readFeedback-{{ $order->feedback->id }}"
                                data-whatever="@mdo">Read Feedback</button>
                        @else
                            <button type="button" class="btns btn-sm ml-1 text-color custom-mobile-font-size p-2"
                                data-toggle="modal" data-target="#mobile-showFeedback-{{ $order->id }}"
                                data-whatever="@mdo">Write Feedback</button>
                        @endif
                    @else
                        <button type="button"
                            class="btns btn-sm text-color custom-mobile-font-size p-2">Pending</button>
                    @endif
                </div>
            </div>
            {{-- modal --}}
            @if ($order->clearing)
                <div class="modal fade" id="mobile-clearing-{{ $order->clearing->id }}" tabindex="-1" role="dialog"
            ...
            {{-- modal --}}
            @if ($order->form)
                <div class="modal fade" id="mobile-form-{{ $order->form->id }}" tabindex="-1" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content background-color-modals modal-border">
                            <div class="modal-body">
            ...
            @if (isset($order->feedback))
                <div class="modal fade" id="mobile-readFeedback-{{ $order->feedback->id }}" tabindex="-1"
                    role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content background-color-modals modal-border">
                            <div class="modal-body">
                                <div class="form-group">
                                    <label class="text-color custom-font-size">Your Feedback to this Transition:
                                    </label>
                                    <p class="ml-3 mr-3 text-justify text-color">
                                        {{ $order->feedback->body ? $order->feedback->body : 'NO TEXT' }} </p>
            ...
            {{-- end modal --}}
            {{-- modal --}}
            <div class="modal fade" id="mobile-showFeedback-{{ $order->id }}" tabindex="-1" role="dialog"
                aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content background-color-modals modal-border">
                        <div class="modal-body">
                            <div class="form-group">
                                <form action=" {{ route('customer.feedbacks.store') }} " method="post">
                                    ...
                                    <textarea class="form-control form-control-sm background-color-inputs border-0" id="body" rows="5"
                                    ...
                                    <div class="d-flex justify-content-end mt-3">
                                        <button type="button" class="btn color pr-4 pl-4 mr-1 btn-sm custom-font-size"
                                            data-dismiss="modal">Close</button>
                                        <button type="submit"
                                            class="btns color pr-4 pl-4 btn-sm custom-font-size">Submit</button>
                                    </div>
                                </form>
                            </div>
                        </div>
