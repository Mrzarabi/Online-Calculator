    ...
    <tbody class="tb">
        ...
        @foreach ($orders as $order)
            <tr class="with-bottom-linear-gradient-to-left">
                ...
                <td class="text-color border-top-0">
                    {{ isset($order->element->name) ? $order->element->name : 'NO TEXT' }}
                ...
                <td class="text-color border-top-0">
                    <div class="d-flex justify-content-center mb-2">
                        <button type="button" class="btns p-2 custom-font-size text-color mr-1" data-toggle="modal"
                            data-target="#order-{{ $order->id }}" data-whatever="@mdo">Delete</button>
                        <a href=" {{ route('clearing.create', ['order' => $order->id]) }} "
                            class="btns text-color p-2 custom-font-size mr-1">{{ $order->clearing ? 'Update Doc' : 'Add Doc' }}</a>
                        @if ($order->clearing)
                            <button type="button" class="btns text-color p-2 custom-font-size mr-1" data-toggle="modal"
                                data-target="#clearing-{{ $order->clearing->id }}" data-whatever="@mdo">Show
                                Doc</button>
                        @endif
                        <button type="button" class="btns text-color p-2 custom-font-size mr-1" data-toggle="modal"
                            data-target="#user-{{ $order->user->id }}" data-whatever="@mdo">User Info</button>
                        @if ($order->form)
                            <button type="button" class="btns text-color p-2 custom-font-size mr-1" data-toggle="modal"
                                data-target="#form-{{ $order->form->id }}" data-whatever="@mdo">Show Form</button>
                        @endif
