            ...
            <tbody class="tb">
                ...
                @foreach ($orders as $order)
                    <tr class="with-bottom-linear-gradient-to-left">
                        ...
                        <td class="border-top-0 text-color">
                            {{ $order->element->name ? $order->element->name : 'NO TEXT' }}
                            {{ $order->output_number ? $order->output_number : 'NO TEXT' }}
                            {{ $order->output_currency_unit ? $order->output_currency_unit : 'NO TEXT' }}</td>
                        <td class="border-top-0 text-color">{{ Carbon::parse($order->created_at)->format('d/m/Y') }}
                        </td>
                        <td class="border-top-0 text-color">
                            <div class="d-flex justify-content-center mb-2">
                                @if ($order->clearing)
                                    <button type="button" class="btns btn-sm mr-1 text-color custom-font-size p-2"
                                        data-toggle="modal" data-target="#clearing-{{ $order->clearing->id }}"
                                        data-whatever="@mdo">Show Doc</button>
                                @endif
                                @if ($order->form)
                                    <button type="button" class="btns btn-sm mr-1 text-color custom-font-size p-2"
                                        data-toggle="modal" data-target="#form-{{ $order->form->id }}"
                                        data-whatever="@mdo">Show Form</button>
                                @endif
                                @if ($order->accept == true)
                                    <button type="button"
                                        class="btns btn-sm text-color custom-font-size p-2">Accepted</button>

                                    @if ($order->feedback)
                                        <button type="button" class="btns btn-sm ml-1 text-color custom-font-size p-2"
                                            data-toggle="modal" data-target="#readFeedback-{{ $order->feedback->id }}"
                                            data-whatever="@mdo">Read Feedback</button>
                                    @else
                                        <button type="button" class="btns btn-sm ml-1 text-color custom-font-size p-2"
                                            data-toggle="modal" data-target="#showFeedback-{{ $order->id }}"
                                            data-whatever="@mdo">Write Feedback</button>
                                    @endif
                                @else
                                    <button type="button"
                                        class="btns btn-sm text-color custom-font-size p-2">Pending</button>
                                @endif
                            </div>
                        </td>
                    </tr>
                    {{-- modal --}}
                    @if ($order->clearing)
                        <div class="modal fade" id="clearing-{{ $order->clearing->id }}" tabindex="-1"
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <h5 class="ml-3 mr-3 text-color text-justify">
                                                {{ $order->clearing->title ? $order->clearing->title : 'NO TEXT' }}
                                            </h5>
                                        </div>
                                        <div class="form-group">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content background-color-modals modal-border">
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            <label class="custom-font-size color">User {{ $order->calculator->name }}
                                                wallet: </label>
                                            <h6 class="ml-3 mr-3 text-justify mb-3 text-color">
                                        ...
                                        @isset($order->form->telegram)
                                            <div class="form-group">
                                                ...
                                                <h6 class="ml-3 mr-3 text-justify text-color"> {{ $order->form->telegram }}
                                                </h6>
                                            </div>
                                        @endisset
                                        ...
                                        @isset($order->form->extra)
                                            <div class="form-group">
                                                <label class="custom-font-size color">Extra note on your user transaction:
                                                </label>
                    ...
                    @if (isset($order->feedback))
                        <div class="modal fade" id="readFeedback-{{ $order->feedback->id }}" tabindex="-1"
                            role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content background-color-modals modal-border">
                                    <div class="modal-body">
                    ...
                    {{-- end modal --}}
                    {{-- modal --}}
                    <div class="modal fade" id="showFeedback-{{ $order->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content background-color-modals modal-border">
                                <div class="modal-body">
                                    <div class="form-group">
                                        <form action=" {{ route('customer.feedbacks.store') }} " method="post">
                                            ...
                                            <div class="d-flex justify-content-end mt-3">
                                                <button type="button"
                                                    class="btn text-color pr-4 pl-4 mr-1 btn-sm custom-font-size"
                                                    data-dismiss="modal">CLOSE</button>
                                                <button type="submit"
                                                    class="btns text-color pr-4 pl-4 btn-sm custom-font-size">Submit</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
